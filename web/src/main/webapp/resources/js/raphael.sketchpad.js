...
var hasTouchSupport = "ontouchstart" in document.documentElement || (/iphone|ipad|ipod|android|blackberry|mini|windowssce|palm/i.test(navigator.userAgent.toLowerCase()));
...
(function(Raphael) {
	...
	var SketchPad = function(paper, options) {
		...
		self.regionVisible = function(value) {
			if (value === undefined) {
				return _options.regionVisible;
			}
			_options.regionVisible = value;
			return self; // function-chaining
		};

		self.regions = function() {
			return _options._regions;
		};
		...
		self.json = function(value) {
			if (value === undefined) {
				...
				for (var i = 0, n = _strokes.length; i < n; i++) {
					var stroke = jQuery.extend(true, {}, _strokes[i]);
					_jsonStrokes[i] = stroke;
					if (typeof stroke.path == "object") {
						stroke.path = svg_path_to_string(stroke.path);
					}
				}
				return JSON.stringify(_jsonStrokes);
			}
		...
		function _fire_strokedrawn(stroke) {
			if (!jQuery.isArray(stroke.path)) {
				stroke.path = string_to_svg_path(stroke.path);
			}
		...
		function _fire_patchstroke(stroke) {
			return _patchstroke_fn(stroke);
		}
		...
		function _mouseup(e) {
			...
			if (path != null) {
				...
				var readraw = _fire_strokedrawn(stroke);
