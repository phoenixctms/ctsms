...
public class ProbandLetterPDFBlock {
	...
	private String getGenderSpecificSalutation() {
		if (address != null && address.getProband().getGender() != null) {
			switch (address.getProband().getGender().getSex()) {
				case MALE:
				case TRANSGENDER_MALE:
					return L10nUtil.getProbandLetterPDFLabel(Locales.PROBAND_LETTER_PDF, ProbandLetterPDFLabelCodes.MALE_SALUTATION, PDFUtil.DEFAULT_LABEL);
				case FEMALE:
				case TRANSGENDER_FEMALE:
					return L10nUtil.getProbandLetterPDFLabel(Locales.PROBAND_LETTER_PDF, ProbandLetterPDFLabelCodes.FEMALE_SALUTATION, PDFUtil.DEFAULT_LABEL);
				default:
					break;
			}
		}
	...
	private String getProbandName(boolean withTitles, boolean withFirstName) {
		...
		if (address != null && (proband = address.getProband()) != null) {
